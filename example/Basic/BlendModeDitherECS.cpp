// BlendMode Dither ECS Example - Bayer Ordered Dither Demo
//
// Demonstrates the "Dither" blend mode: a 4×4 Bayer matrix is used in the fragment
// shader to decide whether to discard a fragment based on its alpha value.
// This gives an ordered-dither transparency effect with NO alpha blending.
// Solid3D pipeline (depth write on) – no blend, dithering handled by the compositor.
//
...
class BlendModeDitherECSApp : public WorkObject
{
private:
...
private:
    ...
    bool InitPlaneGridResources()
    {
        ...
        // Dither uses Solid3D: no alpha blend, 4×4 Bayer discard is done in compositor shader
    ...
    bool InitializeECS()
    {
...

        {
            ...
            for (int i = 0; i < kBillboardCount; ++i)
            {
                ...
                const std::string name = "DitherBillboard_" + std::to_string(i);
...
public:
    ...
    bool Init() override
    {
        std::cout << "\n===== BLEND MODE DITHER ECS INIT =====\n" << std::endl;
        std::cout << "BlendMode: Dither (4x4 Bayer ordered dither)" << std::endl;
        std::cout << "Pipeline:  Solid3D (depth write on, no blend, dither-discard in shader)" << std::endl;
...
};//class BlendModeDitherECSApp

int os_main(int argc, os_char** argv)
{
    return RunFramework<BlendModeDitherECSApp>(OS_TEXT("BlendMode Dither ECS - Bayer Ordered Dither Demo"), argc, argv, 1280, 720);
}
