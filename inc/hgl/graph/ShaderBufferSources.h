...
namespace hgl::graph::mtl
{
    ...
    // std430(scalar 等价——全标量成员)布局断言：显式 offsetof 检查。
    // 6×uint32/float 头部（24B，4B 连续）+ 8×uint64 基址（8B 对齐，自 24 起）= 88B。
    ...
    #undef HGL_MDP_OFFSET_FIELD
        ...
        {
            offsetof(MeshDrawParams, index_base),      // 0
            offsetof(MeshDrawParams, vertex_base),     // 4
            offsetof(MeshDrawParams, is_indexed),      // 8
            offsetof(MeshDrawParams, total_vertices),  // 12
            offsetof(MeshDrawParams, char_height),     // 16
            offsetof(MeshDrawParams, first_instance),  // 20
            offsetof(MeshDrawParams, addr_position),   // 24
            offsetof(MeshDrawParams, addr_uv),         // 32
            offsetof(MeshDrawParams, addr_ntb),        // 40
            offsetof(MeshDrawParams, addr_color),      // 48
            offsetof(MeshDrawParams, addr_luminance),  // 56
            offsetof(MeshDrawParams, addr_transform_id),//64
            offsetof(MeshDrawParams, addr_size),       // 72
            offsetof(MeshDrawParams, addr_index),      // 80
        };
