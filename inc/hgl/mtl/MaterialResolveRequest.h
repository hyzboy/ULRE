...
/// 在渲染收集前根据 recipe_id + Geometry GVF 自动解析 MI。
///
/// 解析路径：recipe_id → MaterialRecipeStore → MaterialRecipe → MaterialKey → Program/DMB/MI
...
namespace hgl::graph
{
    ...
    struct MaterialResolveRequest
    {
        ...
        {
            return dirty && recipe_id != mtl::kInvalidMaterialRecipeID;
        }
