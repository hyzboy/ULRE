...
namespace hgl::graph::mtl
{
    ...
    struct MaterialVariantKey
    {
        ...
        /// Full identity hash — includes position_provider so the shader-program cache can
        /// distinguish a 2D (VAB_Vec2) program from a 3D (DirectVec3) one with identical
        /// surface/vertex bits.
        ...
        /// Registry-identity hash — excludes position_provider because it is a runtime
        ...
        bool RegistryEquals(const MaterialVariantKey &rhs) const noexcept
        {
            return variant_row_name_hash == rhs.variant_row_name_hash
                && surface_type == rhs.surface_type
                && render_phase == rhs.render_phase
