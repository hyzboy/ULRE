...
//
//   PresetQualityEntry { quality_level, SurfaceId, vs_override?, fs_override? }
//               一行 = 一个质量档。同一 preset 的多行按 quality_level 降序排列。
//
//   MaterialPresetTable::Lookup(preset, quality_level)
//               遍历该 preset 的表，返回 quality_level <= 请求值的最高匹配；
//               若全部条目高于请求值则返回最低质量（末尾）条目。
//
// NOTE:
//   RenderPhase 不是这张表的维度。Surface 函数对所有 phase 不变；
//   phase 差异（compositor 模板选择 / 管线状态）由上层 PassShaderResolver 处理。
...
public:
    ...
    /// 返回 (preset, quality_level) 的最佳匹配条目。
    /// 从请求 quality 向下回退，找第一个 quality_level <= 请求值的条目。
    /// 若该 preset 没有任何条目则返回 nullptr。
