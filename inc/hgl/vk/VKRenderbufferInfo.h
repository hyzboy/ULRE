...
public:
    ...
    RenderbufferInfo(const std::vector<VkFormat> &cl,const VkFormat ds):RenderbufferInfo(cl.data(),static_cast<uint32_t>(cl.size()),ds){}
...
public:
    ...
    const uint32_t  GetAttachmentCount()const
    {
        return(depth_stencil_format==PF_UNDEFINED
                ?static_cast<uint32_t>(color_format_list.size())
                :static_cast<uint32_t>(color_format_list.size()+1));
    }
    ...
    const uint32_t          GetColorCount ()const{return static_cast<uint32_t>(color_format_list.size());}
