...
public:
    ...
    {
        vkCmdPushConstants(cmd_buf,layout,(VkShaderStageFlagBits)hgl::graph::kAllGraphicsOrMesh,0,size,data);
    }

    void PushConstants(const void *data,const uint32_t size)                        {vkCmdPushConstants(cmd_buf,pipeline_layout,(VkShaderStageFlagBits)hgl::graph::kAllGraphicsOrMesh,0,       size,data);}
    void PushConstants(const void *data,const uint32_t offset,const uint32_t size)  {vkCmdPushConstants(cmd_buf,pipeline_layout,(VkShaderStageFlagBits)hgl::graph::kAllGraphicsOrMesh,offset,  size,data);}
