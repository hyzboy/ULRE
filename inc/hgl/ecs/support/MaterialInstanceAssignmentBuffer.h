...
namespace hgl::ecs
{
    ...
    public:
         ...
         */
        void BindMaterialInstanceID(graph::ShaderMaterialProgram* mtl) const;

        /**
         * 绑定材质实例数据到材质
         */
        void BindMaterialInstance(graph::ShaderMaterialProgram* mtl) const;

        /**
         * 绑定MaterialInstanceTextureID SSBO到材质（TextureArray用）
         * 当材质实例包含 MIT 数据时自动填充并绑定；无 MIT 数据时为空操作。
         */
        void BindMaterialInstanceTextureID(graph::ShaderMaterialProgram* mtl) const;
