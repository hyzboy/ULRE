...
- 2026-03-04 头文件解耦收敛（渲染公共头去除 ShaderGen 实现依赖）
  ...
  - [inc/hgl/shadergen/ShaderDescriptorInfo.h](inc/hgl/shadergen/ShaderDescriptorInfo.h) 已去除对 `MaterialDescriptorInfo.h` 的头级依赖，改为直接依赖中立 `VKShaderDescriptor.h`（并移除未使用的 `StringList.h`），将 descriptor 指针类型来源下沉到 vk 基础层。
  - [inc/hgl/shadergen/MaterialCreateInfo.h](inc/hgl/shadergen/MaterialCreateInfo.h) 已去除对 `ShaderCreateInfoVertex.h` / `ShaderCreateInfoGeometry.h` / `ShaderCreateInfoFragment.h` 的头级 include，改为前置声明；`RenderOptions.h` / `RenderTargetOutputConfig.h` / `ShaderBufferSource.h` 的冗余 include 已移除。
  - [src/ShaderGen/MaterialCreateInfo.cpp](src/ShaderGen/MaterialCreateInfo.cpp) 已补齐实现级显式 include（`ShaderCreateInfoVertex/Geometry/Fragment`），确保“头前置声明 + 源文件落地依赖”分层成立。
  - [inc/hgl/shadergen/ShaderCreateInfo.h](inc/hgl/shadergen/ShaderCreateInfo.h) 已移除未使用的 `StringList.h` 与 `ShaderVariableType.h` include，进一步降低基础 ShaderCreateInfo 头的传递依赖。
  - [inc/hgl/shadergen/ShaderCreateInfoMap.h](inc/hgl/shadergen/ShaderCreateInfoMap.h) 已移除对 `ShaderCreateInfo.h` 的头级直接 include，改为前置声明 `ShaderCreateInfo`，并将 `Add(ShaderCreateInfo*)` 下沉到 [src/ShaderGen/ShaderCreateInfoMap.cpp](src/ShaderGen/ShaderCreateInfoMap.cpp) 实现。
  - [inc/hgl/shadergen/ShaderCreateInfoMap.h](inc/hgl/shadergen/ShaderCreateInfoMap.h) 已进一步移除对 `VK.h` 的依赖，改为 `ShaderStage` 前置声明（`enum class ShaderStage:uint32_t`）+ `<cstdint>`，继续压缩 Vulkan 头传递面。
  - [src/ShaderGen/CMakeLists.txt](src/ShaderGen/CMakeLists.txt) 已将 `ShaderCreateInfoMap.cpp` 纳入 `SHADER_CREATER_SOURCE_FILES`，保证实现单元参与编译。
  - [src/SceneGraph/module/MaterialCreatePrecheckAdapter.cpp](src/SceneGraph/module/MaterialCreatePrecheckAdapter.cpp) 已移除对 `ShaderCreateInfoMap.h` 的重复 include，统一通过 `MaterialCreateInfo.h` 获取 map 类型声明。
  - [inc/hgl/graph/module/ShaderGenSPVModuleAdapter.h](inc/hgl/graph/module/ShaderGenSPVModuleAdapter.h) 已去除对 `ShaderCreateInfoMap.h` 与 `VKShaderModule.h` 的头级直接 include，改为前置声明 `ShaderCreateInfo/ShaderCreateInfoMap/ShaderModule` + `VK.h` 轻量依赖；实现侧 [src/SceneGraph/module/ShaderGenSPVModuleAdapter.cpp](src/SceneGraph/module/ShaderGenSPVModuleAdapter.cpp) 已显式补齐 `ShaderCreateInfoMap.h`。
  - [inc/hgl/graph/module/ShaderGenDescriptorPolicyAdapter.h](inc/hgl/graph/module/ShaderGenDescriptorPolicyAdapter.h) 已移除对 `ShaderGenDescriptorLayoutAdapter.h` 的头级直接 include，改为直接依赖 `ShaderGenContract.h` + `VKShaderDescriptor.h` + `string/vector`；实现侧 [src/SceneGraph/module/ShaderGenDescriptorPolicyAdapter.cpp](src/SceneGraph/module/ShaderGenDescriptorPolicyAdapter.cpp) 已补齐布局适配器 include。
  - 新增轻量日志粒度契约 [inc/hgl/graph/module/ShaderGenDiffLogDetail.h](inc/hgl/graph/module/ShaderGenDiffLogDetail.h)，将 `DiffLogDetail` 从 `RendererShaderGenAdapter` 内部枚举抽离为模块级独立类型。
  - [inc/hgl/graph/module/RendererShaderGenAdapter.h](inc/hgl/graph/module/RendererShaderGenAdapter.h) 已切换为 `using DiffLogDetail = ShaderGenDiffLogDetail` 兼容别名，保留原有调用面不破坏。
  - [inc/hgl/graph/module/ShaderGenContractPathContext.h](inc/hgl/graph/module/ShaderGenContractPathContext.h) 与 [inc/hgl/graph/module/ShaderGenReadOnlyValidationGate.h](inc/hgl/graph/module/ShaderGenReadOnlyValidationGate.h) 已去除对 `RendererShaderGenAdapter.h` 的头级依赖，改为依赖独立 `ShaderGenDiffLogDetail.h`；对应 `.cpp` 已完成类型替换与实现级 include 补齐。
  - 新增轻量验证/统计 DTO 头 [inc/hgl/graph/module/ShaderGenValidationTypes.h](inc/hgl/graph/module/ShaderGenValidationTypes.h)，承载 `ShaderGenValidationReport/Record` 与 `ShaderGenProfilerSnapshot`，并在 [inc/hgl/graph/module/RendererShaderGenAdapter.h](inc/hgl/graph/module/RendererShaderGenAdapter.h) 通过 `using` 兼容原嵌套类型名。
  - [inc/hgl/graph/module/MaterialManager.h](inc/hgl/graph/module/MaterialManager.h) 与 [inc/hgl/graph/core/GraphicsContext.h](inc/hgl/graph/core/GraphicsContext.h) 已从 `RendererShaderGenAdapter.h` 头级依赖切换到 `ShaderGenValidationTypes.h` / `ShaderGenDiffLogDetail.h`，并将 ShaderGen profiler/validation 包装函数改为 `.cpp` 实现。
  - 对应实现侧 [src/SceneGraph/module/MaterialManager.cpp](src/SceneGraph/module/MaterialManager.cpp) 与 [src/SceneGraph/render/GraphicsContext.cpp](src/SceneGraph/render/GraphicsContext.cpp) 已补齐 `RendererShaderGenAdapter.h` include 与新签名对齐。
  - 调用面去旧名： [src/SceneGraph/module/ShaderGenReadOnlyValidationGate.cpp](src/SceneGraph/module/ShaderGenReadOnlyValidationGate.cpp) 与 [test/RendererShaderGenAdapterStrictGateTest.cpp](test/RendererShaderGenAdapterStrictGateTest.cpp) 已改为直接使用 `ShaderGenValidationReport`，减少外部代码对 `RendererShaderGenAdapter::*` 嵌套类型名的依赖。
  - [src/SceneGraph/module/RendererShaderGenAdapter.cpp](src/SceneGraph/module/RendererShaderGenAdapter.cpp) 内部实现类型已统一迁移到 `ShaderGenValidationReport/Record` 与 `ShaderGenProfilerSnapshot`，当前代码面已无 `RendererShaderGenAdapter::ValidationReport* / ProfilerSnapshot` 旧类型名残留（仅头内兼容别名保留）。
  - [inc/hgl/graph/module/RendererShaderGenAdapter.h](inc/hgl/graph/module/RendererShaderGenAdapter.h) 已移除 `ValidationReport/ValidationReportRecord/ProfilerSnapshot` 兼容 `using` 别名，接口签名改为直接公开 `ShaderGenValidationReport/Record` 与 `ShaderGenProfilerSnapshot`，兼容层进一步减薄。
  - [inc/hgl/graph/module/RendererShaderGenAdapter.h](inc/hgl/graph/module/RendererShaderGenAdapter.h) 已继续移除 `DiffLogDetail` 兼容别名；adapter 接口与实现已统一为直接使用 `ShaderGenDiffLogDetail`。
  - 新增 fallback bridge： [inc/hgl/graph/module/ShaderGenValidationQueryBridge.h](inc/hgl/graph/module/ShaderGenValidationQueryBridge.h) / [src/SceneGraph/module/ShaderGenValidationQueryBridge.cpp](src/SceneGraph/module/ShaderGenValidationQueryBridge.cpp)，承接 ShaderGen profiler/validation 的无 `MaterialManager` 场景查询。
  - [src/SceneGraph/render/GraphicsContext.cpp](src/SceneGraph/render/GraphicsContext.cpp) 已改为通过 `ShaderGenValidationQueryBridge` 调用 fallback，不再直接 include/调用 `RendererShaderGenAdapter` 静态接口。
  - [src/SceneGraph/CMakeLists.txt](src/SceneGraph/CMakeLists.txt) 已纳入 `ShaderGenValidationQueryBridge` 头/源文件到 module 构建列表。
  - 新增存储服务： [inc/hgl/graph/module/ShaderGenValidationStorageService.h](inc/hgl/graph/module/ShaderGenValidationStorageService.h) / [src/SceneGraph/module/ShaderGenValidationStorageService.cpp](src/SceneGraph/module/ShaderGenValidationStorageService.cpp)，集中承载 profiler sample 累积与 validation report 历史存取。
  - [src/SceneGraph/module/RendererShaderGenAdapter.cpp](src/SceneGraph/module/RendererShaderGenAdapter.cpp) 已将静态查询/存储职责委托到 storage service，adapter 本体收敛为“验证编排 + 结果写入”职责。
  - [src/SceneGraph/CMakeLists.txt](src/SceneGraph/CMakeLists.txt) 已纳入 `ShaderGenValidationStorageService` 头/源文件到 module 构建列表。
  - [src/SceneGraph/module/ShaderGenValidationQueryBridge.cpp](src/SceneGraph/module/ShaderGenValidationQueryBridge.cpp) 已由“经 `RendererShaderGenAdapter` 中转”切换为“直接委托 `ShaderGenValidationStorageService`”，fallback 查询链路与 adapter 完全解耦。
  - 新增 report utility： [inc/hgl/graph/module/ShaderGenValidationReportUtils.h](inc/hgl/graph/module/ShaderGenValidationReportUtils.h) / [src/SceneGraph/module/ShaderGenValidationReportUtils.cpp](src/SceneGraph/module/ShaderGenValidationReportUtils.cpp)，统一承载 `AddError/AddWarning/Merge/ApplyContractResult` 等报告操作。
  - [src/SceneGraph/module/RendererShaderGenAdapter.cpp](src/SceneGraph/module/RendererShaderGenAdapter.cpp) 已切换为调用 `ShaderGenValidationReportUtils`，删除本地重复报告操作实现，进一步缩小 adapter 文件职责面。
  - `overall_valid` 重算规则已进一步收敛到 `ShaderGenValidationReportUtils`：新增 `RecomputeShaderGenValidationOverallValid(...)`，并替换 [src/SceneGraph/module/RendererShaderGenAdapter.cpp](src/SceneGraph/module/RendererShaderGenAdapter.cpp) 内重复内联公式，避免业务规则散落。
  - [src/SceneGraph/CMakeLists.txt](src/SceneGraph/CMakeLists.txt) 已纳入 `ShaderGenValidationReportUtils` 头/源文件。
  - 新增 diff presenter： [inc/hgl/graph/module/ShaderGenMirrorDiffPresenter.h](inc/hgl/graph/module/ShaderGenMirrorDiffPresenter.h) / [src/SceneGraph/module/ShaderGenMirrorDiffPresenter.cpp](src/SceneGraph/module/ShaderGenMirrorDiffPresenter.cpp)，集中承载 mirror diff 汇总、DiffKV 日志输出与 profiler sample 记录。
  - [src/SceneGraph/module/RendererShaderGenAdapter.cpp](src/SceneGraph/module/RendererShaderGenAdapter.cpp) 已移除内联 `PrintLegacyMirrorDiff` 与相关静态辅助函数，改为委托 `PresentShaderGenMirrorDiff(...)`，adapter 进一步靠拢 orchestration。
  ...
  - `ShaderGenContractGateReporter` 已去除对 `RendererShaderGenAdapter` 的直接依赖：外部错误报告写入下沉到 `ShaderGenValidationStorageService::StoreExternalShaderGenValidationError(...)`，adapter 侧 `RecordExternalValidationError(...)` 也改为复用该 storage API，避免报告构造逻辑双份维护。
  - [inc/hgl/graph/module/RendererShaderGenAdapter.h](inc/hgl/graph/module/RendererShaderGenAdapter.h) 已移除未再被调用的公共接口 `RecordExternalValidationError(...)`，并同步删除 [src/SceneGraph/module/RendererShaderGenAdapter.cpp](src/SceneGraph/module/RendererShaderGenAdapter.cpp) 对应实现，进一步收敛 adapter 对外 API 面。
  - [inc/hgl/graph/module/ShaderGenContractGateReporter.h](inc/hgl/graph/module/ShaderGenContractGateReporter.h) / [src/SceneGraph/module/ShaderGenContractGateReporter.cpp](src/SceneGraph/module/ShaderGenContractGateReporter.cpp) 已新增可替换日志 sink（`Set/ResetShaderGenContractGateLogSink`），并将 reporter 内日志输出统一经 sink 分发；默认实现仍写 `stderr`，行为保持不变。
  - gate category/fallback 文本常量已集中到 [inc/hgl/graph/module/ShaderGenContractGateReporter.h](inc/hgl/graph/module/ShaderGenContractGateReporter.h)（`StrictGate.Prebuild/Spv/Vertex/Descriptor`、`layout mismatch/build failed`），并替换 [src/SceneGraph/module/MaterialBuildFlowAdapter.cpp](src/SceneGraph/module/MaterialBuildFlowAdapter.cpp)、[src/SceneGraph/module/MaterialManager.cpp](src/SceneGraph/module/MaterialManager.cpp)、[src/SceneGraph/module/ShaderGenContractGateReporter.cpp](src/SceneGraph/module/ShaderGenContractGateReporter.cpp) 中散落字面量。
  - `contract_path_decision_histogram` 的 key 已集中到 [inc/hgl/graph/module/ShaderGenValidationStorageService.h](inc/hgl/graph/module/ShaderGenValidationStorageService.h)（`spv/vertex/descriptor` 的 `use_mirror/use_legacy/strict_abort`），并替换 [src/SceneGraph/module/MaterialBuildFlowAdapter.cpp](src/SceneGraph/module/MaterialBuildFlowAdapter.cpp) 内统计上报字面量，减少观测语义漂移风险。
  - `mirror-preferred build aborted` 文案拼接已收敛：新增 `BuildMirrorPreferredAbortReason(...)` 与前缀常量 `kShaderGenMirrorPreferredAbortPrefix`（[inc/hgl/graph/module/ShaderGenContractGateReporter.h](inc/hgl/graph/module/ShaderGenContractGateReporter.h)、[src/SceneGraph/module/ShaderGenContractGateReporter.cpp](src/SceneGraph/module/ShaderGenContractGateReporter.cpp)），并替换 [src/SceneGraph/module/MaterialBuildFlowAdapter.cpp](src/SceneGraph/module/MaterialBuildFlowAdapter.cpp) 中 3 处重复拼接。
  - [src/SceneGraph/module/ShaderGenContractGateReporter.cpp](src/SceneGraph/module/ShaderGenContractGateReporter.cpp) 已新增通用 fallback helper（`ReportMirrorFallbackWithReason/WithPhaseReason`），`SPV/Vertex/Descriptor` fallback 报告改为复用通用路径，去除重复的安全文本与日志分发代码。
  - [src/SceneGraph/module/MaterialBuildFlowAdapter.cpp](src/SceneGraph/module/MaterialBuildFlowAdapter.cpp) 已新增 `ReportStrictAbortDecision(...)`，统一 strict-abort 分支的“决策计数 + abort reason 组装 + 上报 + 返回”流程，替换 SPV/Vertex/Descriptor 三处重复片段。
  - [src/SceneGraph/module/MaterialBuildFlowAdapter.cpp](src/SceneGraph/module/MaterialBuildFlowAdapter.cpp) 已新增 `RecordBinaryDecision(...)`，统一 `UseLegacy/UseMirror` 的二分决策计数上报，替换 Vertex/Descriptor 两处重复计数分支。
  - [src/SceneGraph/module/MaterialBuildFlowAdapter.cpp](src/SceneGraph/module/MaterialBuildFlowAdapter.cpp) 已新增 `AppendShaderModules(...)`，统一 mirror/legacy 模块批量写入 `ShaderModuleMap` 的循环逻辑，进一步收敛 flow 内重复片段。
  - [src/SceneGraph/module/MaterialBuildFlowAdapter.cpp](src/SceneGraph/module/MaterialBuildFlowAdapter.cpp) 已新增 `CollectLegacyDescriptors(...)` 并替换 `BuildMaterialBindingsFlow(...)` 内联 descriptor 收集块，进一步缩短主流程函数体。
