...
- **S5 ✅（2026-09-26，示例 stats 接通 + GPU 侧"整级 vs 条带"对拍工具落地；对拍**发现真实差异、根因未收敛**）**：
  ...
  - **人工验收（用户 2026-09-26 手动 WASD 行走，未用自动行走）**：向前移动**无可见接缝**；
    ...
    **三组数字自洽 ⇒ 条带滚动在按设计工作**（`shift_x/shift_y` 各自必须 `=0` 或 `±B` 才走滚动，
    ...
    （与 S4 的 1.8× seam 余量结论一致），但仍是内容不一致 ⇒ 保留 S6 待定位。
- **S6（新，S5 直出）**：定位"条带重画 vs 整级重建"内容差异的根因。
  ...
  - **关键线索（新）**：差异 bbox=(509,186)-(1023,614)，**不含新画的条带**（条带在 `x∈[0,16)`，
    那里 0 差异）⇒ **条带写入是精确的；差异全部落在"更早跨格时写进缓存、此后一直没被重画"的
    老内容上** ⇒ 符合"某次跨格的写入/清除把脏数据烘进缓存、直到下次整级重建才消失"的形态
    （`A缺 233`＝老内容缺了几何，正是被多清的纹素或错位写入的签名）。
  - **下一步（待定）**：① 定向对拍——条带帧读回**该条带矩形**并与整级重建的同一矩形逐纹素比
    （当前 bbox 已间接说明它精确，此步为确认）；② 审计每次跨格的 `rect` 覆盖是否**恰好**等于
    新暴露区（多 1 纹素即会清掉仍有效的缓存 ⇒ 与 `A缺` 签名吻合），可先在 CPU 侧回放 400 次跨格
    断言 `rect 面积 == B×M` 且与 `cache_offset` 的增量严格对应；③ 若确认"多清/错位"在 GPU 侧
    清除路径（`clear_scissor_depth` 的 rect 用了 *物理* 坐标而 scissor 用了另一坐标系），
    修 `EnvironmentSystem` 的 rect 计算。
  - **可选收口**：差异占 0.03% 纹素、只在剪影、且用户手动行走**看不到接缝**（与 S4 的 1.8× 余量
    一致）⇒ 若 S6 继续追的收益低于成本，可"记录为已知有界差异"收口（需用户裁决）。
