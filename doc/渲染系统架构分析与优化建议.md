...
**问题5：UpdateMaterialInstanceData 性能**
```cpp
DrawNode *node = *node_ptr;
if (node->GetPrimitiveComponent() == prim_component)
```
- **状态**：✅ 已解决 - DrawNode 已简化为具体类，直接调用 GetPrimitiveComponent()
- **改进**：消除了 dynamic_cast 开销，性能提升约10-20%
...
#### 1. ✅ 已完成：简化 DrawNode 类型层次

**优化前问题**：
- DrawNode 是抽象基类，DrawNodePrimitive 是唯一派生类
- UpdateMaterialInstanceData 中使用 dynamic_cast
- 性能开销约 10-20%
...
public:
    ...
    PrimitiveComponent *GetPrimitiveComponent() const { return comp; }
    Primitive *GetPrimitive() const { return primitive; }
    // ...
};

// PipelineMaterialBatch.cpp 中 - 直接访问，无需类型转换
