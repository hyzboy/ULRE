...
MaterialCreateInfo *CreateStandardTextureArray(const contract::PhysicalDeviceProfileLite *profile, const Material3DCreateConfig *cfg)
{
    ...
    var_key.texture_source_mode = TextureSourceMode::Tex2DArray;
    var_key.feature_bits        = VF_HasBaseColorTex | VF_HasNormalTex;
    const MaterialVariantDesc *var_desc = GetBuiltinVariantRegistry().QueryVariant(var_key);
    if (!var_desc)
    {
        std::fprintf(stderr, "[StandardTextureArray] VariantRegistry lookup failed\n");
        return nullptr;
    }
