...
static hgl::graph::ColorSource MakeBindlessSource(hgl::mtl::SamplerSlot slot=hgl::mtl::SamplerSlot::Diffuse){hgl::graph::ColorSource cs;cs.kind=hgl::graph::ColorSourceKind::BuiltinBindlessSamplerArray;cs.slot=slot;cs.builtin.output_format=hgl::graph::ColorSourceOutputFormat::RGBA;return cs;}
static void Test1(){using namespace hgl::graph;BuiltinBindlessSamplerArrayCodegen c;auto src=MakeBindlessSource();ResolvedBindings r;ResolvedBinding rb;rb.debug_name="Sampler_Diffuse_Bindless";rb.set=3;rb.binding=0;r.push_back(rb);std::string o;ShaderWriter w(o);c.EmitDeclarations(w,src,r);CHECK_CONTAINS(o,"GL_EXT_nonuniform_qualifier");CHECK_CONTAINS(o,"Sampler_Diffuse_Bindless[]");CHECK_CONTAINS(o,"set=3");CHECK_CONTAINS(o,"binding=0");}
static void Test2(){using namespace hgl::graph;BuiltinBindlessSamplerArrayCodegen c;auto src=MakeBindlessSource();ResolvedBindings r;std::string o;ShaderWriter w(o);c.EmitGetterFunction(w,src,r);CHECK_CONTAINS(o,"nonuniformEXT");CHECK_CONTAINS(o,"GetSamplerDiffuse");CHECK_CONTAINS(o,"Sampler_Diffuse_Bindless");}
