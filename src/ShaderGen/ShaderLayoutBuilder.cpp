...
ShaderLayoutContract BuildShaderLayoutContract(const mtl::MaterialCreateInfo &mci)
{
    ...
    for (size_t si = 0; si < DESCRIPTOR_SET_TYPE_COUNT; ++si)
    {
        ...
        for (auto &kv : sd_set.ubo_descriptor_map)
        {
        ...
        for (auto &kv : sd_set.ssbo_descriptor_map)
        {
            const ShaderDescriptor *sd = kv.second;
            if (!sd || sd->binding < 0)
                continue;
        ...
        for (auto &kv : sd_set.texture_descriptor_map)
        {
            const ShaderDescriptor *sd = kv.second;
            if (!sd || sd->binding < 0)
                continue;
        ...
        for (auto &kv : sd_set.texture_sampler_descriptor_map)
        {
            const ShaderDescriptor *sd = kv.second;
            if (!sd || sd->binding < 0)
                continue;
