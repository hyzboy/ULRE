...
void OutMaterial(const MaterialData *ms,const OSString &filename)
{
    ...
    dos.WriteFully("ShaderMaterialProgram\x1A\x1",10);
    ...
    GLogInfo(OS_TEXT("ShaderMaterialProgram Texture Count ")+OSString(ms->tex_count));
...
void AssimpLoader::LoadMaterial()
{
    ...
    LOG_BR;
    GLogInfo(OS_TEXT("ShaderMaterialProgram Count ")+OSString(scene->mNumMaterials));
    LOG_BR;
...
void AssimpLoader::LoadMesh()
{
    ...
    for(unsigned int i=0;i<scene->mNumMeshes;i++)
    {
        ...
        if(mesh->GetNumUVChannels()>0)
            GLogInfo(mesh_name+U8_TEXT(" ShaderMaterialProgram Index is ")+UTF8String(mesh->mMaterialIndex));
