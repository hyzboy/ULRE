...
namespace hgl::graph{
    ...
    #define SAVE_PIPELINE_TO_FILE(name) do{ PipelineData *pd=BuildPipelineData(mtl::Make##name##Config()); SaveToToml(filesystem::JoinPathWithFilename(pathname,OS_TEXT(#name) OS_TEXT(".pipeline.toml")),*pd); delete pd; }while(0)
