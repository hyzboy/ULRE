#include<hgl/vk/VertexDataManager.h>
#include<hgl/vk/VKVertexAttribBuffer.h>
#include<hgl/graph/module/BufferManager.h>
#include<hgl/vk/VKDevice.h>

DEFINE_LOGGER_MODULE(VertexDataManager)

namespace hgl::graph
{
    VertexDataManager::VertexDataManager(VulkanDevice *dev,const VertexFormatMap &format_map)
    {
        buffer_manager=nullptr;
        device=dev;
        vertex_format_map=format_map;

        vab_max_size=0;
        vab_cur_size=0;
        vab=zero_new<VAB *>(GetVertexAttribCount());

        ibo_cur_size=0;
        ibo=nullptr;

        LogVerbose(OS_TEXT("VertexDataManager constructed: vi_count=") + OSString::numberOf(GetVertexAttribCount()));
    }

    VertexDataManager::VertexDataManager(BufferManager *bm,const VertexFormatMap &format_map)
    {
        buffer_manager=bm;
        device=bm?bm->GetDevice():nullptr;
        vertex_format_map=format_map;

        vab_max_size=0;
        vab_cur_size=0;
        vab=zero_new<VAB *>(GetVertexAttribCount());

        ibo_cur_size=0;
        ibo=nullptr;

        LogVerbose(OS_TEXT("VertexDataManager constructed: vi_count=") + OSString::numberOf(GetVertexAttribCount()));
    }

    VertexDataManager::~VertexDataManager()
    {
        LogVerbose(OS_TEXT("VertexDataManager destroying: vab_cur_size=") + OSString::numberOf(vab_cur_size) + OS_TEXT(", ibo_cur_size=") + OSString::numberOf(ibo_cur_size));

        if (buffer_manager)
        {
            for (uint i=0;i<GetVertexAttribCount();i++)
            {
                if (vab[i])
                    buffer_manager->Release(vab[i]);
            }
            if (ibo)
                buffer_manager->Release(ibo);

            if (vab)
            {
                delete[] vab;
                vab = nullptr;
            }
        }
        else
        {
            SAFE_CLEAR_OBJECT_ARRAY_OBJECT(vab,GetVertexAttribCount());
            SAFE_CLEAR(ibo);
        }

        LogVerbose(OS_TEXT("VertexDataManager destroyed."));
    }

    /**
    * 初始化顶点数据管理器
    * @param vbo_size VAB大小
    * @param ibo_size IBO大小
    * @param index_type 索引类型
    */
    bool VertexDataManager::Init(const VkDeviceSize vbo_size,const VkDeviceSize ibo_size,const IndexType index_type)
    {
        MLogInfo(VertexDataManager, OS_TEXT("Init called: vbo_size=") + OSString::numberOf(vbo_size) + OS_TEXT(", ibo_size=") + OSString::numberOf(ibo_size) + OS_TEXT(", index_type=") + OSString::numberOf((int)index_type));

        if(vab[0]||ibo)     //已经初始化过了
        {
            LogWarning(OS_TEXT("Init skipped: already initialized."));
            return(false);
        }

        if(vbo_size<=0)
        {
            LogError(OS_TEXT("Init failed: invalid vbo_size=") + OSString::numberOf(vbo_size));
            return(false);
        }

        vab_max_size=vbo_size;
        ibo_cur_size=ibo_size;

        vab_cur_size=0;
        ibo_cur_size=0;

        uint i=0;
        for(const auto &entry : vertex_format_map)
        {
            const VkFormat format = entry.second;

            LogVerbose(OS_TEXT("Creating VAB[") + OSString::numberOf(i) + OS_TEXT("] format=") + OSString::numberOf((int)format) + OS_TEXT(", size=") + OSString::numberOf(vab_max_size));

            if (buffer_manager)
                vab[i]=buffer_manager->CreateVAB(format,vab_max_size);
            else
                vab[i]=device->CreateVAB(format,vab_max_size);
            if(!vab[i])
            {
                LogError(OS_TEXT("CreateVAB failed for index=") + OSString::numberOf(i));

                // cleanup any created VABs
                for(uint j=0;j<i;j++)
                {
                    if (vab[j])
                    {
                        if (buffer_manager)
                            buffer_manager->Release(vab[j]);
                        else
                            SAFE_CLEAR(vab[j]);
                    }
                }

                return(false);
            }

            LogVerbose(OS_TEXT("VAB[") + OSString::numberOf(i) + OS_TEXT("] created."));
            ++i;
        }

        vbo_data_chain.Init(vab_max_size);
        LogVerbose(OS_TEXT("vbo_data_chain initialized with size=") + OSString::numberOf(vab_max_size));

        if(ibo_size>0)
        {
            if (buffer_manager)
                ibo=buffer_manager->CreateIBO(index_type,ibo_size,nullptr);
            else
                ibo=device->CreateIBO(index_type,ibo_size);
            if(!ibo)
            {
                LogError(OS_TEXT("CreateIBO failed: size=") + OSString::numberOf(ibo_size));

                // cleanup VABs
                for(uint i=0;i<GetVertexAttribCount();i++)
                {
                    if (buffer_manager)
                        buffer_manager->Release(vab[i]);
                    else
                        SAFE_CLEAR(vab[i]);
                }

                return(false);
            }

            ibo_data_chain.Init(ibo_size);
            MLogInfo(VertexDataManager, OS_TEXT("IBO created and ibo_data_chain initialized with size=") + OSString::numberOf(ibo_size));
        }

        LogVerbose(OS_TEXT("VertexDataManager Init success."));

        return(true);
    }

    BlockAllocator::UserNode *VertexDataManager::AcquireIB(const VkDeviceSize count)
    {
        LogVerbose(OS_TEXT("AcquireIB requested: count=") + OSString::numberOf(count));

        if(count<=0)
        {
            LogWarning(OS_TEXT("AcquireIB: invalid count=") + OSString::numberOf(count));
            return(nullptr);
        }

        BlockAllocator::UserNode *un=ibo_data_chain.Acquire(hgl_align<int>(count,4));

        if(!un)
        {
            LogError(OS_TEXT("AcquireIB failed: no space for count=") + OSString::numberOf(count));
            return(nullptr);
        }

        ibo_cur_size+=un->GetCount();

        LogVerbose(OS_TEXT("AcquireIB success: allocated=") + OSString::numberOf(un->GetCount()) + OS_TEXT(", ibo_cur_size=") + OSString::numberOf(ibo_cur_size));

        return(un);
    }

    bool VertexDataManager::ReleaseIB(BlockAllocator::UserNode *un)
    {
        if(!un)
        {
            LogWarning(OS_TEXT("ReleaseIB called with null node."));
            return(false);
        }

        const auto count=un->GetCount();

        if(!ibo_data_chain.Release(un))
        {
            LogError(OS_TEXT("ReleaseIB failed for count=") + OSString::numberOf(count));
            return(false);
        }

        ibo_cur_size-=count;
        LogVerbose(OS_TEXT("ReleaseIB success: released=") + OSString::numberOf(count) + OS_TEXT(", ibo_cur_size=") + OSString::numberOf(ibo_cur_size));
        return(true);
    }

    BlockAllocator::UserNode *VertexDataManager::AcquireVAB(const VkDeviceSize count)
    {
        LogVerbose(OS_TEXT("AcquireVAB requested: count=") + OSString::numberOf(count));

        if(count<=0)
        {
            LogWarning(OS_TEXT("AcquireVAB: invalid count=") + OSString::numberOf(count));
            return(nullptr);
        }

        BlockAllocator::UserNode *un=vbo_data_chain.Acquire(hgl_align<int>(count,4));

        if(!un)
        {
            LogError(OS_TEXT("AcquireVAB failed: no space for count=") + OSString::numberOf(count));
            return(nullptr);
        }

        vab_cur_size+=un->GetCount();

        LogVerbose(OS_TEXT("AcquireVAB success: allocated=") + OSString::numberOf(un->GetCount()) + OS_TEXT(", vab_cur_size=") + OSString::numberOf(vab_cur_size));

        return(un);
    }

    bool VertexDataManager::ReleaseVAB(BlockAllocator::UserNode *un)
    {
        if(!un)
        {
            LogWarning(OS_TEXT("ReleaseVAB called with null node."));
            return(false);
        }

        const auto count=un->GetCount();

        if(!vbo_data_chain.Release(un))
        {
            LogError(OS_TEXT("ReleaseVAB failed for count=") + OSString::numberOf(count));
            return(false);
        }

        vab_cur_size-=count;
        LogVerbose(OS_TEXT("ReleaseVAB success: released=") + OSString::numberOf(count) + OS_TEXT(", vab_cur_size=") + OSString::numberOf(vab_cur_size));
        return(true);
    }
}//namespace hgl::graph
