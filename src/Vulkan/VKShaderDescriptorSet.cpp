...
namespace
{
    bool TryGetFixedBinding(DescriptorSetType set_type, const char *name, int &out_binding)
    {
        ...
        case DescriptorSetType::PerObject:
            ...
            //（l2w / l2w_index / material_private_data_index）。
            if (strcmp(name, "l2w") == 0)              { out_binding = kPerObjectBindingL2W;           return true; }
            if (strcmp(name, "l2w_index") == 0)        { out_binding = kPerObjectBindingL2WIndex;  return true; }
            if (strcmp(name, "mtl_private_data_index") == 0) { out_binding = kPerObjectBindingPrivateDataIndex; return true; }
