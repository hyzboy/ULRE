...
#define BUFFER_MANAGER_CREATE_BUFFER(name)    VkBufferOwner *BufferManager::Create##name(const AnsiString &buf_name, VkDeviceSize size, void *data, SharingMode sharing_mode, const std::source_location &loc) \
                                              {   \
                                                  VulkanDevice *device = GetDevice(); \
                                                  VkBufferOwner *buf = device->Create##name(buf_name, size, data, BufferAllocPolicy::Auto, sharing_mode, BufferUpdateClass::Default, loc);   \
                                                  \
