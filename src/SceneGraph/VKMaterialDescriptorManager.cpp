...
MaterialDescriptorManager::MaterialDescriptorManager(const AnsiString &name,const ShaderDescriptorSetArray &sds_array)
{
    ...
    ENUM_CLASS_FOR(DescriptorSetType,int,i)
    {
        for(const auto &d:sds_array[i].ubo_descriptor_map) set_values[i].push_back(d.get());
        for(const auto &d:sds_array[i].ssbo_descriptor_map) set_values[i].push_back(d.get());
        for(const auto &d:sds_array[i].texture_descriptor_map) set_values[i].push_back(d.get());
        for(const auto &d:sds_array[i].texture_sampler_descriptor_map) set_values[i].push_back(d.get());
