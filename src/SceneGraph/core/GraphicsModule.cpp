...
namespace hgl::graph
{
    GraphicsModule::GraphicsModule(vk::VulkanDevice* in_device,
    ...
    vk::VulkanDevAttr* GraphicsModule::GetDevAttr() const
    {
        return device ? device->GetDevAttr() : nullptr;
    }

    vk::VulkanPhyDevice* GraphicsModule::GetPhyDevice() const
    {
        return device ? device->GetPhyDevice() : nullptr;
    }
