...
namespace hgl::graph::inline_geometry
{
    ...
    Geometry *CreateWallsFromLines2D(GeometryCreater *pc, const WallCreateInfo *wci)
    {
        ...
        struct VJoin
        {
            std::vector<Vector2f> leftPts;
            std::vector<Vector2f> rightPts;
        };
        ...
        for(size_t vi = 0; vi < nverts; ++vi)
        {
            ...
            
            {
                ...
                if(sPrev.len > 1e-8f && sNext.len > 1e-8f)
                {
                    if(LineIntersect(sPrev.left1, sPrev.dir, sNext.left0, sNext.dir, t, u))
                    {
                        ...
                        float miter_len = sqrt((left_pt.x - verts[seq[vi]].x) * (left_pt.x - verts[seq[vi]].x) +
                                              (left_pt.y - verts[seq[vi]].y) * (left_pt.y - verts[seq[vi]].y));
                                              
                        if(!(miter_len > wci->miterLimit * halfT && wci->cornerJoin == WallCreateInfo::CornerJoin::Miter))
                            left_ok = true;
                    }
                }
            }
            ...
            
            {
                ...
                if(sPrev.len > 1e-8f && sNext.len > 1e-8f)
                {
                    if(LineIntersect(sPrev.right1, sPrev.dir, sNext.right0, sNext.dir, t, u))
                    {
                        ...
                        float miter_len = sqrt((right_pt.x - verts[seq[vi]].x) * (right_pt.x - verts[seq[vi]].x) +
                                              (right_pt.y - verts[seq[vi]].y) * (right_pt.y - verts[seq[vi]].y));
            ...
            if(wci->cornerJoin == WallCreateInfo::CornerJoin::Miter && left_ok && right_ok)
            {
                ...
                sPrev.left1 = left_pt;
                sNext.left0 = left_pt;
                sPrev.right1 = right_pt;
                sNext.right0 = right_pt;
            }
            ...
            else // Round
            {
                ...
                if(cross < 0.0f)
                {
                    ...
                    for(int s = 0; s <= segs; s++)
                    {
                        ...
                        vjoin[vi].leftPts.push_back(Vector2f(verts[seq[vi]].x + cos(ang) * halfT,
                    ...
                    if(!vjoin[vi].leftPts.empty())
                    {
                        sPrev.left1 = vjoin[vi].leftPts.front();
                        sNext.left0 = vjoin[vi].leftPts.back();
                    }
        ...
        for(size_t vi = 0; vi < nverts; ++vi)
        {
            ...
            if(lcount == 0)
            {
                vjoin[vi].leftPts.push_back(vjoin[vi].rightPts.front());
                lcount = 1;
            }
            
            if(rcount == 0)
            {
                vjoin[vi].rightPts.push_back(vjoin[vi].leftPts.front());
                rcount = 1;
            }
            ...
            else if(lcount == 1 && rcount > 1)
            {
                ...
                {
                    left_poly.push_back(vjoin[vi].leftPts[0]);
                    right_poly.push_back(vjoin[vi].rightPts[k]);
                }
            }
        ...
        if(left_poly.size() != right_poly.size())
            return nullptr;
        ...
        std::vector<Vector3f> finalVerts;
        finalVerts.reserve(m * 6);
        
        std::vector<Vector2f> finalUV;
        finalUV.reserve(m * 6);
        ...
        for(size_t i = 1; i < nverts; i++)
            accum[i] = accum[i - 1] + segs[i - 1].len;
        ...
        for(size_t i = 0; i < m; i++)
        {
            ...
            finalUV.push_back(Vector2f(u, 0.0f));
            finalUV.push_back(Vector2f(u, 1.0f));
        }

        // indices
        std::vector<uint32_t> finalIndices;
        finalIndices.reserve((m - (closed ? 0 : 1)) * 18 + 12);
        
        auto vertIndex = [&](size_t i, int corner) -> uint32_t
        {
            return (uint32_t)(i * 6 + corner);
        };
        
        size_t segCount = closed ? m : (m - 1);

        // decide if vertical faces need per-face independent vertices
        bool flatSide = (wci->cornerJoin == WallCreateInfo::CornerJoin::Miter ||
        ...
        for(size_t i = 0; i < segCount; i++)
        {
            ...
            finalIndices.push_back(t0);
            finalIndices.push_back(t2);
            finalIndices.push_back(t1);
            
            finalIndices.push_back(t0);
            finalIndices.push_back(t3);
            finalIndices.push_back(t2);

            // Left vertical face
            uint32_t orig_l0 = vertIndex(i, 0), orig_l1 = vertIndex(i, 3);
            uint32_t orig_l2 = vertIndex(ni, 3), orig_l3 = vertIndex(ni, 0);
            ...
            if(flatSide)
            {
                ...
                finalVerts.push_back(finalVerts[orig_l0]);
                finalVerts.push_back(finalVerts[orig_l1]);
                finalVerts.push_back(finalVerts[orig_l2]);
                finalVerts.push_back(finalVerts[orig_l3]);
                
                finalUV.push_back(finalUV[orig_l0]);
                finalUV.push_back(finalUV[orig_l1]);
                finalUV.push_back(finalUV[orig_l2]);
                finalUV.push_back(finalUV[orig_l3]);
                
                finalIndices.push_back(base + 0);
                finalIndices.push_back(base + 1);
            ...
            if(flatSide)
            {
                ...
                finalVerts.push_back(finalVerts[orig_r0]);
                finalVerts.push_back(finalVerts[orig_r1]);
                finalVerts.push_back(finalVerts[orig_r2]);
                finalVerts.push_back(finalVerts[orig_r3]);
                
                finalUV.push_back(finalUV[orig_r0]);
                finalUV.push_back(finalUV[orig_r1]);
                finalUV.push_back(finalUV[orig_r2]);
                finalUV.push_back(finalUV[orig_r3]);
        ...
        if(!closed)
        {
            ...
            uint32_t capL0 = vertIndex(0, 4), capR0 = vertIndex(0, 5);
            uint32_t sR0 = vertIndex(0, 2), sL0 = vertIndex(0, 3);
            
            finalIndices.push_back(capL0);
            finalIndices.push_back(capR0);
            finalIndices.push_back(sR0);
            
            finalIndices.push_back(capL0);
            finalIndices.push_back(sR0);
            finalIndices.push_back(sL0);

            // end end - only top cap
            size_t li = m - 1;
            uint32_t ebase = vertIndex(li, 0);
            uint32_t capL1 = vertIndex(li, 4), capR1 = vertIndex(li, 5);
            uint32_t sR1 = vertIndex(li, 2), sL1 = vertIndex(li, 3);
            
            finalIndices.push_back(capL1);
            finalIndices.push_back(capR1);
            finalIndices.push_back(sR1);
            
            finalIndices.push_back(capL1);
            finalIndices.push_back(sR1);
            finalIndices.push_back(sL1);
        }
        ...
        for(size_t ti = 0; ti + 2 < finalIndices.size(); ti += 3)
        {
            ...
            vertNormals[ia].z += N.z;
            
            vertNormals[ib].x += N.x;
            vertNormals[ib].y += N.y;
            vertNormals[ib].z += N.z;
            
            vertNormals[ic].x += N.x;
            vertNormals[ic].y += N.y;
            vertNormals[ic].z += N.z;
        }
        ...
        for(size_t i = 0; i < m; i++)
        {
            ...
            if(capL < vertNormals.size())
                vertNormals[capL] = Vector3f(0, 0, 1);
                
            if(capR < vertNormals.size())
                vertNormals[capR] = Vector3f(0, 0, 1);
        }
