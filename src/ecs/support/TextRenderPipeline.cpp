...
namespace hgl::ecs
{
                                                                                   ...
                                                                                   uint32_t estimate_chars)
    {
        ...
        guard.material = material_manager->AcquireMaterialInternal(graph::mtl::MaterialPreset::Text2D, &mtl_cfg);
        if (!guard.material)
            return nullptr;
        ...
        if (mi_bytes > 0)
        {
            ...
            if (!guard.material->BindSSBO(graph::mtl::SSBODescriptorSemantic::MaterialInstanceData,
                                           ...
                                           graph::VulkanDevice* device)
    {
        for (auto& pair : inputs)
        {
            ...
            if (resources->material_handle == graph::InvalidMaterialInstanceHandle)
            {
                ...
                init.domain_handle = material_manager->GetMRDManager()->GetHandle(domain);
