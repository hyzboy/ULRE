...
#ifdef _DEBUG
        MaterialInstanceAssignmentDebugStats debug_stats{};
#endif
...
#ifdef _DEBUG
                ResolveMaterialInstanceFromStateWithDebugStats(item, diagnostics, &debug_stats);
#else
                ResolveMaterialInstanceFromStateWithDebugStats(item, diagnostics);
#endif
...
#ifdef _DEBUG
        LogMaterialInstanceAssignmentDebugSummary("StatMaterialInstance", diagnostics, debug_stats, item_count, unique_mi_count);

        if (diagnostics)
        {
            diagnostics->RecordR3MaterialInstanceAssignmentStats(
                static_cast<uint64_t>(debug_stats.items_seen),
                static_cast<uint64_t>(debug_stats.triad_present),
                static_cast<uint64_t>(debug_stats.triad_bridge_compare_total),
                static_cast<uint64_t>(debug_stats.triad_bridge_compare_equal),
                static_cast<uint64_t>(debug_stats.triad_bridge_rescue_candidate),
                static_cast<uint64_t>(debug_stats.triad_bridge_override_candidate),
                static_cast<uint64_t>(debug_stats.shadow_switch_eval_total),
                static_cast<uint64_t>(debug_stats.shadow_switch_would_change));
        }
#endif
